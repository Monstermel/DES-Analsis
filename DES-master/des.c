...
int message_expansion[] = {32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,
...
void process_message(unsigned char* message_piece,
                     ...
                     int mode) {
    // Esto es mala practica, lo optimo es limitar lo mas posible el tiempo de
    // vida de las variables
    int i, k;
    int shift_size;
    unsigned char shift_byte;

    unsigned char initial_permutation[8];  // 64 bits
    ...
    unsigned char ln[4], rn[4];   // n-esima L y R
    unsigned char er[6], ser[4];  // E(R_n) y S(E(R_n) ^ K_n)

    // Rondas
    int key_index;
    for (k = 1; k <= 16; k++) {
        memcpy(ln, r, 4);  // L_(n+1) := R_(n)

        memset(er, 0, 6);  // Inicializacion de E(R_n)
        ...
        // Inicializacion de f(R_n, K_n)
        for (i = 0; i < 4; i++) {
            rn[i] = 0;
        }

        // P(S(E(R_n) ^ K_n)) o f(R_n, K_n)
        for (i = 0; i < 32; i++) {
        ...
        // Actualizacion de los valores de la siguiente ronda
        for (i = 0; i < 4; i++) {
            l[i] = ln[i];
            r[i] = rn[i];
        }
    ...
    // (R_16,L_16) := (L_16,R_16) (Intercambio)
