...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        printf(
            "You must provide at least 1 parameter, where you specify the "
            "action.");
        return 1;
    }
    ...
    if (strcmp(argv[1], ACTION_GENERATE_KEY) == 0) {  // Generate key file
        if (argc != 3) {
            printf(
                "Invalid # of parameter specified. Usage: run_des -g "
                "keyfile.key");
            return 1;
        }
        ...
        if (argc != 5) {
            printf(
                "Invalid # of parameters (%d) specified. Usage: run_des "
                "[-e|-d] keyfile.key input.file output.file",
                argc);
            return 1;
        }
        ...
        unsigned char* des_key = (unsigned char*)malloc(8 * sizeof(char));
        bytes_read =
            fread(des_key, sizeof(unsigned char), DES_KEY_SIZE, key_file);
        ...
        unsigned char* processed_block =
            (unsigned char*)malloc(8 * sizeof(char));
        ...
        while (fread(data_block, 1, 8, input_file)) {
            ...
            if (block_count == number_of_blocks) {
                if (process_mode == ENCRYPTION_MODE) {
                    ...
                    if (padding <
                        8) {  // Fill empty data block bytes with padding
                        memset((data_block + 8 - padding),
